...
class GameCore:  # expected 2 blank lines
    def __init__(self, target_words, guess_words, quordle=True) -> None:
        # list of valid 5 letter words
        # assert isinstance(valid_words, list) or isinstance(valid_words, tuple)  # line too long
        ...
        # gui interface class
        self.gui = GUI()
        # words tried so far  # trainling whitespace
        self.tries = []
        self.keyboard_use = {}
        # this determines the layout the keyboard is print
        self.keyboard_layout = ["qwertzuiopü",  # trailing whitespace
                                "asdfghjklöä",  # trailing whitespace
                                "yxcvbnmß"]
    ...
    def set_up_keyboard_use(self):
        ...
        """
        self.keyboard_use = {"ä": [0,0,0,0],  # missing whitespace after ,
                        "ö": [0,0,0,0],  # continuation line under-indented for visual indent + missing whitespace after ,
                        "ü": [0,0,0,0],  # continuation line under-indented for visual indent + missing whitespace after ,
                        "ß": [0,0,0,0],}  # continuation line under-indented for visual indent + missing whitespace after ,

        for i in range(26):
            self.keyboard_use[chr(ord("a") + i)] = [0,0,0,0]  # missing whitespace after ,
        return self.keyboard_use

    def update_keyboard_letter(self, letter, wordle_index, use, downgrade=False):  # line too long
        ...
        for word_index, word in enumerate(self.tries):
            for letter_index, letter in enumerate(word):
                ...
                for i in range(4):
                    ...
                    # === sequence mod start ===
                    # if were not playing wordle, meaning were playing sequence,  # line too long
                    # we stop updating the letters for sub-wordles after the
                    # one that were matching
                    if not self.quordle and unmatched or \
                        not self.quordle and self.wordles[i].matched:  # continuation line with same indent as next logical line
    ...
    def setup(self):
        ...
        for _ in range(4):
            # find an unused index  # trailing whitespace
            while 42:
            ...
            if not quordle and not wordle.matched:
                for _ in range(3-index):
                    ...
                    for _ in range(matchlen):
                        templist.append((0,0,0,0,0))  # missing whitespace after ,
    ...
    def game_loop(self):  # too many blank lines
        # setup
        self.setup()
        self.gui.clear_screen()
        ...
        while 42:
            ...
            while 42:
                ...
                # make sure its a valid try  # trailing whitespace
                if len(new_try) != 5:
            ...
            if self.wordles[0].matched and \
                ...
                self.wordles[3].matched:  # continuation line with same indent as next logical line
                # print the latest info on the gamestate
                # self.update_gui()
